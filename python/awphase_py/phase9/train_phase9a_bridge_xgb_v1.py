...
def pick_features(df, label_col, group_col):
    excluded = set(LEAK_OR_ID_COLUMNS) | {label_col, group_col}
    features = []
    constant = []
    skipped = []
...
def main():
    ...
    neg = int((y == 0).sum())
    if pos == 0 or neg == 0:
        raise SystemExit(f"Need both classes. Found positives={pos}, negatives={neg}")
    ...
    metrics = {
        ...
        "excluded_leak_or_id_columns": sorted(LEAK_OR_ID_COLUMNS),
