...
tail -n +2 "$MANIFEST" | while IFS=$'\t' read -r SPLIT CHR START END LABEL _READS _SNP _BEDINT _BAM PANEL _JSON _TRUTH_VCF _TRUTH_BED; do
  ...
  OUT="$ROOT/phase9a_block_bridge_candidates.tsv"
  LOG="results/phase9/logs/make_phase9a_${LABEL}.log"

  echo
  echo "===== Phase9A bridge candidates: $LABEL ====="
